...
export function HeroStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl bg-white/10 px-4 py-3 ring-1 ring-white/15 backdrop-blur-sm transition-colors duration-200 hover:bg-white/15">
      <p className="font-display text-[10px] font-bold uppercase tracking-wider text-white/50">{label}</p>
      <p className="mt-1 font-tabular text-2xl font-bold text-white">{formatPercent(value)}</p>
    </div>
  )
}
...
export function InsightBlock({
  ...
  return (
    <button
      ...
      disabled={!onSelect}
      className={cn(
        'group relative w-full overflow-hidden rounded-xl p-4 text-left ring-1 ring-[color:var(--border-subtle)] transition-[box-shadow,border-color,background-color] duration-200 hover:ring-mck-sky/30 hover:shadow-[var(--shadow-glow)]',
        onSelect && 'insight-block-interactive cursor-pointer',
...
export function FormulaLine({ step, text, highlight }: { step: string; text: string; highlight?: boolean }) {
  return (
    <div
      className={cn(
        'rounded-lg px-3 py-2.5 font-tabular text-xs ring-1 transition-colors',
        highlight
